...
def weighted_loss(logits, labels, number_class, frequency):
    ...
    cross_entropy = tf.nn.weighted_cross_entropy_with_logits(labels=label_onehot, logits=logits_reshape,
                                                             pos_weight=frequency)
    cross_entropy_mean = tf.reduce_mean(input_tensor=cross_entropy, name='cross_entropy')
    tf.compat.v1.summary.scalar('loss', cross_entropy_mean)
    correct_prediction = tf.equal(tf.argmax(input=logits_reshape, axis=-1), label_flatten)
    accuracy = tf.reduce_mean(input_tensor=tf.cast(correct_prediction, dtype=tf.float32))
    tf.compat.v1.summary.scalar('accuracy', accuracy)

    return cross_entropy_mean, accuracy, tf.argmax(input=logits_reshape, axis=-1)
...
def normal_loss(logits, labels, number_class):
    ...
    function!
    """
    label_flatten = tf.cast(tf.reshape(labels, [-1]), dtype=tf.int64)
    logits_reshape = tf.reshape(logits, [-1, number_class])
...
def train_op(total_loss, opt):
    ...
    with tf.control_dependencies(update_ops):
        if (opt == "ADAM"):
            optimizer = tf.compat.v1.train.AdamOptimizer(0.001, epsilon=0.0001)
            print("Running with Adam Optimizer with learning rate:", 0.001)
        elif (opt == "SGD"):
            base_learning_rate = 0.1
            learning_rate = tf.compat.v1.train.exponential_decay(base_learning_rate, global_step, decay_steps=1000,
                                                                 decay_rate=0.0005)
        ...
        grads = optimizer.compute_gradients(total_loss, var_list=tf.compat.v1.trainable_variables())
